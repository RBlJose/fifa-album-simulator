...
const C = {
  ...
  gold: "#f0c040", goldDim: "#c49a20",
  blue: "#2196f3", green: "#22c55e",
  orange: "#f97316", red: "#ef4444",
  purple: "#a855f7", teal: "#14b8a6",
  muted: "#4a607a", text: "#ddeaf8", dim: "#7a9ab8",
};
...
function expTotal(n)         { return n * harmonicN(n); }
function expPacks(n, spp)    { return expTotal(n) / spp; }
function expCost(n, spp, p)  { return expPacks(n, spp) * p; }
function inflPacks(n, spp)   { return (n * Math.LN2) / spp; }
function remCostFn(k, n, spp, price) {
  if (k >= n) return 0;
  return (n * harmonicN(n - k) / spp) * price;
}
function remPacksFn(k, n, spp) {
  if (k >= n) return 0;
  return n * harmonicN(n - k) / spp;
}
function costPerNew(k, n, price, spp) {
  return (price / spp) * (n / Math.max(1, n - k));
}
function crossK(n, price, spp, indP) {
  return Math.max(0, Math.min(n - 1, Math.round(n * (1 - price / (indP * spp)))));
}
function dupsAtK(k, n) {
  if (k <= 0) return 0;
  let d = 0;
  for (let i = 1; i <= Math.min(k, n - 1); i++) d += n / (n - i + 1);
  return Math.max(0, Math.round(d - k));
}
function estimateUnique(total, n) {
  return Math.min(n, Math.round(n * (1 - Math.pow(1 - 1 / n, total))));
}
function rareCost(n, spp, price, nRare, rareP) {
  return +((expCost(n - nRare, spp, price)) + nRare * rareP).toFixed(2);
}
function tradeWindow(n, spp, price, indP) {
  return {
    minK:   Math.round(n * 0.50),
    idealK: Math.round(n * 0.75),
    cK:     crossK(n, price, spp, indP),
  };
}
function tradeScore(k, n, spp, price, indP) {
  const { minK, idealK, cK } = tradeWindow(n, spp, price, indP);
  if (k < minK)    return { score: Math.round(k / minK * 30),                    label: "Muy pocos repetidos aún",           color: C.red,    ready: false };
  if (k < idealK)  return { score: 30 + Math.round((k - minK) / (idealK - minK) * 50), label: "Puedes empezar a intercambiar", color: C.orange, ready: true  };
  if (k <= cK)     return { score: 95,                                             label: "¡Momento óptimo!",                  color: C.green,  ready: true  };
  if (k <= cK * 1.1) return { score: 65,                                           label: "Aún útil, mejor sueltos",           color: C.gold,   ready: true  };
  return               { score: 35,                                                 label: "Repetidos ya no tan útiles",        color: C.muted,  ready: false };
}

// ── Datos para gráficos ────────────────────────────────────────────────────
function makeGrowthData(n, spp) {
  const inf = Math.round(inflPacks(n, spp));
  const maxP = Math.round(inf * 5);
  const step = Math.max(1, Math.round(maxP / 70));
  const rows = [];
  for (let i = 0; i * step <= maxP; i++) {
    const packs = i * step;
    const t = packs * spp;
    const unique = n * (1 - Math.pow(1 - 1 / n, t));
    rows.push({
      packs,
      unique: Math.min(Math.round(unique), n),
      dups:   Math.round(Math.max(0, t - unique)),
    });
  }
  return rows;
}

function makeBudgetData(n, spp, price) {
  const mean = expTotal(n);
  const std  = Math.sqrt(n * n * (Math.PI ** 2 / 6) - n * harmonicN(n));
  const maxB = Math.ceil(expCost(n, spp, price) * 2.2);
  const step = Math.max(1, Math.round(maxB / 50));
  const rows = [];
  for (let i = 0; i * step <= maxB; i++) {
    const budget = i * step;
    const z      = ((budget / price) * spp - mean) / std;
    rows.push({
      budget: +budget.toFixed(1),
      prob:   Math.round(Math.min(100, Math.max(0, 100 / (1 + Math.exp(-1.7 * z))))),
    });
  }
  return rows;
}

function makeStageData(n, spp, price) {
  const stages = [
    ["0→50%", 0, 0.5], ["50→75%", 0.5, 0.75], ["75→90%", 0.75, 0.9],
    ["90→95%", 0.9, 0.95], ["95→99%", 0.95, 0.99], ["99→100%", 0.99, 1],
  ];
  return stages.map(([label, s, e]) => {
    const k1 = Math.max(1, Math.floor(s * n) + 1);
    const k2 = Math.floor(e * n);
    let draws = 0;
    for (let k = k1; k <= k2; k++) draws += n / (n - k + 1);
...
function runSimFromK(startK, n, spp) {
  const col = new Uint8Array(n);
  let placed = 0;
  while (placed < startK) {
    const s = Math.floor(Math.random() * n);
    if (!col[s]) { col[s] = 1; placed++; }
  }
  let u = startK, p = 0;
  while (u < n) {
    p++;
...
function buildResults(packs, price) {
  const N      = packs.length;
  const sorted = [...packs].sort((a, b) => a - b);
  const avg    = arr => arr.reduce((a, b) => a + b, 0) / arr.length;
  const pct    = p => sorted[Math.floor(p * N / 100)];
  const mn = sorted[0], mx = sorted[N - 1], B = 22;
  const bs = Math.max(1, (mx - mn) / B);
  ...
  return {
    n: N, mean: Math.round(avg(sorted)), median: pct(50),
...
// ── UI Components ──────────────────────────────────────────────────────────
function NumInput({ label, raw, setRaw, min, max, step, accent, width = 82 }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <input
        ...
        style={{
          width, background: accent ? "#0f2010" : "#0f1e35",
          border: `1px solid ${accent || C.muted}`, borderRadius: 7,
          color: accent || C.gold, padding: "6px 8px",
...
function KPI({ icon, label, val, sub, accent = C.gold, glow = false, small = false }) {
  return (
    <div style={{
...
function SimChart({ hist, mean, price, height = 170 }) {
  return (
      ...
      <BarChart data={hist} barSize={16}>
        ...
        <Tooltip
          content={({ active, payload, label }) => {
            ...
            return (
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "9px 12px", fontSize: 11 }}>
...
export default function App() {
  ...
  const EP            = Math.round(expPacks(n, spp));
  const EC            = +expCost(n, spp, price).toFixed(2);
  const INF           = Math.round(inflPacks(n, spp));
  const CK            = crossK(n, price, spp, indPrice);
  const pctCross      = +(CK / n * 100).toFixed(1);
  const remPacks      = Math.round(remPacksFn(kUnique, n, spp));
  const remCost       = +remCostFn(kUnique, n, spp, price).toFixed(2);
  const remCostInd    = +(missing * indPrice).toFixed(2);
  const remCostLow    = +(missing * Math.max(0.10, indPrice * 0.6)).toFixed(2);
  const remCostHigh   = +(missing * Math.min(3.00, indPrice * 3.0)).toFixed(2);
  ...
  const ECRare        = rareCost(n, spp, price, nRare, rarePrice);
  const rarePremium   = +(nRare * rarePrice).toFixed(2);
  const { minK, idealK, cK } = tradeWindow(n, spp, price, indPrice);
  ...
  const runSimProgress = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setSimProg(buildResults(res.map(r => r.packs), price));
      setRunningP(false);
    }, 30);
  }, [kUnique, n, spp, price]);
  ...
  const runSim = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setSim(buildResults(res.map(r => r.packs), price));
      setRunning(false);
    }, 30);
  }, [n, spp, price, simN]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "Georgia,serif", paddingBottom: 40 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#060d1b,#0c1e3a,#060d1b)", borderBottom: `1px solid ${C.border}`, padding: "20px 24px 16px", position: "relative", overflow: "hidden" }}>
        ...
        {[0, 1, 2, 3].map(i => (
          <div key={i} style={{ position: "absolute", borderRadius: "50%", width: 50 + i * 40, height: 50 + i * 40, border: `1px solid ${C.gold}${(14 - i * 3).toString(16)}`, right: -8 + i * 7, top: -8 + i * 5, pointerEvents: "none" }} />
        ))}
        <div style={{ maxWidth: 940, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
            ...
            <span style={{ background: `${C.gold}22`, border: `1px solid ${C.goldDim}`, color: C.gold, padding: "5px 10px", borderRadius: 20, fontSize: 10, alignSelf: "flex-end", marginBottom: 2 }}>
              Ecuador 🇪🇨
            </span>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "20px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 20 }}>
          ...
          <KPI icon="✨" label={`Total + ${nRare} raros`} val={`$${ECRare}`}     sub={`+$${rarePremium} por raros`}   accent={C.purple}      />
        ...
        {tab === "progress" && (
          <div>
            ...
            <div style={{ background: C.card, border: `2px solid ${C.teal}44`, borderRadius: 14, padding: 18, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                {[["unique", "Tengo X únicos"], ["total", "Compré X cromos en total"]].map(([m, l]) => (
                  <button key={m} onClick={() => setInputMode(m)} style={{ padding: "6px 13px", borderRadius: 8, border: `1px solid ${inputMode === m ? C.teal : C.border}`, background: inputMode === m ? `${C.teal}22` : "transparent", color: inputMode === m ? C.teal : C.dim, fontSize: 11, cursor: "pointer", fontWeight: inputMode === m ? 700 : 400 }}>
                    {l}
                  </button>
                ))}
              </div>
              <div style={{ display: "flex", gap: 14, alignItems: "flex-end", flexWrap: "wrap" }}>
                <NumInput
                  label={inputMode === "unique" ? `🎴 Únicos (de ${n})` : "🛍️ Total comprados"}
                  raw={rawOwned} setRaw={setRawOwned}
                  min={0} max={inputMode === "unique" ? n : n * 15} step={1} accent={C.green}
                />
                ...
                <NumInput label="♻️ Repetidos reales (opcional)" raw={rawDups} setRaw={setRawDups} min={0} max={50000} step={1} accent={C.orange} />
              </div>
              <div style={{ marginTop: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[["social", "🤝 Sí, sin problema"], ["maybe", "🤔 Si es fácil"], ["solo", "🙅 Prefiero no"]].map(([m, l]) => (
                    <button key={m} onClick={() => setSocialMode(m)} style={{ padding: "6px 13px", borderRadius: 8, border: `1px solid ${socialMode === m ? C.teal : C.border}`, background: socialMode === m ? `${C.teal}22` : "transparent", color: socialMode === m ? C.teal : C.dim, fontSize: 11, cursor: "pointer", fontWeight: socialMode === m ? 700 : 400 }}>
                      {l}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 14 }}>
              ...
              <div style={{ position: "relative", marginBottom: 12 }}>
                <div style={{ background: C.border, borderRadius: 8, height: 18, overflow: "hidden", position: "relative" }}>
                  <div style={{ position: "absolute", left: `${minK / n * 100}%`, width: `${(cK - minK) / n * 100}%`, height: "100%", background: `${C.green}30` }} />
            ...
            {/* RECOMENDACIÓN */}
            <div style={{ background: `linear-gradient(135deg,${rec.color}18,${rec.color}06)`, border: `2px solid ${rec.color}66`, borderRadius: 16, padding: 20, marginBottom: 16 }}>
            ...
            {missing > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ color: C.gold, fontWeight: 700, fontSize: 13, marginBottom: 10 }}>💰 ¿Cuánto te costaría completar desde aquí?</div>
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 18 }}>
                  <div style={{ fontSize: 11, color: C.dim, marginBottom: 14 }}>
                    Te faltan <strong style={{ color: C.gold }}>{missing} cromos</strong>. Estas son tus opciones:
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                    <div style={{ background: `${C.orange}11`, border: `1px solid ${C.orange}44`, borderRadius: 10, padding: 14 }}>
                      ...
                      <div style={{ fontSize: 10, color: C.dim, marginTop: 4 }}>~{remPacks} sobres más × ${price}</div>
                    </div>
                    <div style={{ background: `${C.purple}11`, border: `1px solid ${C.purple}44`, borderRadius: 10, padding: 14 }}>
                      ...
                      <div style={{ fontSize: 10, color: C.dim, marginTop: 4 }}>{missing} × ${indPrice} — cero repetidos</div>
                    </div>
                  </div>
                  <div style={{ background: `${C.border}60`, borderRadius: 10, padding: 14 }}>
                    <div style={{ fontSize: 11, color: C.gold, fontWeight: 700, marginBottom: 10 }}>
                      📊 Rango si compras los {missing} sueltos según precio de mercado
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                      {[
                        [`Precio mínimo\n($${(indPrice * 0.6).toFixed(2)}/cromo)`, `$${remCostLow}`,  C.green,  "Lotes o mercado secundario"],
                        [`Precio medio\n($${indPrice}/cromo)`,                      `$${remCostInd}`, C.gold,   "Tu precio configurado"],
                        [`Precio máximo\n($${Math.min(3, indPrice * 3).toFixed(2)}/cromo)`, `$${remCostHigh}`, C.red, "Cromos escasos"],
                    ...
                    <div style={{ marginTop: 10, fontSize: 11, color: C.dim, lineHeight: 1.6 }}>
                      💡 <strong style={{ color: C.text }}>Lo más inteligente:</strong> intercambia lo que puedas (~{tradeable} cromos = ~${savingTrade} ahorrado), compra sueltos los que encuentres a buen precio, y usa sobres solo si estás por debajo del {pctCross}%.
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 14 }}>
              <KPI icon="♻️" label="Repetidos"          val={(dupsReal !== null ? dupsReal : dupsExp).toLocaleString()} sub={dupsReal !== null ? "Ingresados por ti" : "Estimado"} accent={C.orange} small />
              <KPI icon="🔁" label="Intercambiables"    val={tradeable}          sub={`de los ${missing} que faltan`}          accent={C.teal}   small glow />
              <KPI icon="💸" label="Ahorro intercambio" val={`~$${savingTrade}`} sub={`${tradeable} cromos × $${indPrice}`}    accent={C.green}  small />
            </div>

            {/* Simulación desde progreso */}
            <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12 }}>
              <button
                onClick={runSimProgress}
                disabled={runningP || missing === 0}
                style={{ padding: "10px 22px", borderRadius: 10, border: "none", cursor: (runningP || missing === 0) ? "not-allowed" : "pointer", background: (runningP || missing === 0) ? C.muted : C.teal, color: C.bg, fontWeight: 800, fontSize: 13, boxShadow: (runningP || missing === 0) ? "none" : `0 0 14px ${C.teal}55` }}
              >
                {missing === 0 ? "✅ ¡Álbum completo!" : runningP ? "⏳ Simulando..." : "🎲 Simular cromos restantes"}
              </button>
            ...
            {simProg && (
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14 }}>
                ...
                <SimChart hist={simProg.hist} mean={simProg.mean} price={price} />
              </div>
            )}
          </div>
        )}
        ...
        {tab === "trading" && (
          <div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20, marginBottom: 16, display: "grid", gridTemplateColumns: "auto 1fr", gap: 24, alignItems: "center" }}>
              <Gauge score={tScore} label={tLabel} color={tColor} />
              <div>
                <div style={{ fontWeight: 700, color: tColor, fontSize: 14, marginBottom: 10 }}>Tu situación: {kUnique} únicos ({pctDone}%)</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {[
                    ...
                    ["Posición vs ventana",     kUnique < minK ? "Muy temprano" : kUnique <= cK ? "✓ En la ventana" : "Pasaste el cruce", tColor],
                  ].map(([l, v, c]) => (
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 18, marginBottom: 14 }}>
              <div style={{ position: "relative", height: 58, marginBottom: 22 }}>
                ...
                <div style={{ position: "absolute", top: 18, left: `${idealK / n * 100}%`, width: `${(cK - idealK) / n * 100}%`, height: 8, background: `${C.green}99` }} />
                <div style={{ position: "absolute", top: 18, left: `${cK / n * 100}%`, right: 0, height: 8, background: `${C.gold}44`, borderRadius: "0 4px 4px 0" }} />
                <div style={{ position: "absolute", top: 12, left: `${Math.min(parseFloat(pctDone), 99.5)}%`, transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  <div style={{ width: 2, height: 8, background: C.gold }} />
                </div>
                {[
                  [0, "0%", C.muted],
                  [minK / n * 100, `${Math.round(minK / n * 100)}%\nMín`, C.red],
                  [idealK / n * 100, `${Math.round(idealK / n * 100)}%\n★Ideal`, C.green],
                  [cK / n * 100, `${pctCross}%\nCruce`, C.orange],
                  [100, "100%", C.muted],
                ].map(([pos, lbl, col]) => (
                  <div key={pos} style={{ position: "absolute", top: 30, left: `${pos}%`, transform: "translateX(-50%)", textAlign: "center", whiteSpace: "pre-line", fontSize: 9, color: col, fontWeight: 700, lineHeight: 1.3 }}>{lbl}</div>
                ))}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
                {[
                  [C.red,    "Muy temprano",  "Pocos repetidos"],
                  [C.orange, "Empieza",       "Intercambios pequeños"],
                  [C.green,  "★ Ideal",       "Máximo provecho"],
                  [C.gold,   "Pasaste cruce", "Mejor sueltos"],
                ].map(([col, title, desc]) => (
                  <div key={title} style={{ background: `${col}11`, border: `1px solid ${col}33`, borderRadius: 9, padding: "10px 11px" }}>
            ...
            <div style={{ background: `${tColor}11`, border: `1px solid ${tColor}44`, borderRadius: 12, padding: 16 }}>
              ...
              <div style={{ fontSize: 11, color: C.text, lineHeight: 1.7 }}>
                ...
                {kUnique >= minK && kUnique <= cK && `¡Sí, ahora! Tienes ~${dupsUsable.toLocaleString()} repetidos. ~${tradeable} son útiles para intercambiar directamente. Cada intercambio exitoso = $${indPrice} ahorrado.`}
                {kUnique > cK && `Ya pasaste el mejor momento (${pctCross}%). Comprar los ${missing} restantes sueltos ($${remCostInd}) es más barato que seguir con sobres ($${remCost}).`}
              </div>
            </div>
          </div>
        )}
        ...
        {tab === "rarity" && (
          <div>
            ...
            <div style={{ color: C.dim, fontSize: 11, marginBottom: 18 }}>Los raros (el "00", ediciones especiales) se consiguen sueltos. El modelo los trata como compra directa, no vía sobres.</div>
            ...
            <div style={{ background: C.card, border: `2px solid ${C.purple}44`, borderRadius: 14, padding: 18, marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-end" }}>
                <NumInput label="✨ Cuántos cromos raros" raw={rawNRare} setRaw={setRawNRare} min={0} max={50} step={1} width={70} />
                <NumInput label="💎 Precio del raro ($)"  raw={rawRarePrice} setRaw={setRawRarePrice} min={0.5} max={20} step={0.25} width={80} />
                <div style={{ flex: 1, minWidth: 180, paddingBottom: 2 }}>
                  <div style={{ fontSize: 10, color: C.dim, marginBottom: 5 }}>Precio del cromo raro</div>
                  <input type="range" min={0.5} max={10} step={0.25} value={parseFloat(rawRarePrice) || 2} onChange={e => setRawRarePrice(e.target.value)} style={{ width: "100%", accentColor: C.purple }} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 18 }}>
              <KPI icon="🎯" label="Raros a buscar sueltos"    val={nRare}           sub="No los busques en sobres"        accent={C.purple} />
              <KPI icon="💎" label={`Costo de ${nRare} raros`} val={`$${rarePremium}`} sub={`${nRare} × $${rarePrice} c/u`} accent={C.gold}   glow />
              <KPI icon="📊" label="Total con raros incluidos" val={`$${ECRare}`}    sub={`$${EC} + $${rarePremium}`}      accent={C.orange} />
            </div>

            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: `${C.border}80` }}>
                    {["Escenario", "Raros", "Precio c/u", "Costo raros", "Total", "Extra"].map(h => (
                ...
                <tbody>
                  {[
                    ...
                    const tc    = rareCost(n, spp, price, nr, rp);
                    const extra = +(tc - EC).toFixed(2);
                    const isMe  = nr === nRare && Math.abs(rp - rarePrice) < 0.1;
                    return (
                      <tr key={label} style={{ borderTop: `1px solid ${C.border}`, background: isMe ? `${C.purple}08` : "transparent" }}>
                        <td style={{ padding: "9px 14px", fontWeight: 700, color: isMe ? C.gold : C.text, fontSize: 12 }}>{isMe ? "→ " : ""}{label}</td>
                        ...
                        <td style={{ padding: "9px 14px" }}>
                          <span style={{ background: extra > 0 ? `${C.orange}22` : `${C.green}22`, color: extra > 0 ? C.orange : C.green, padding: "3px 8px", borderRadius: 6, fontSize: 10, fontWeight: 700 }}>
                            {extra > 0 ? `+$${extra}` : "Base"}
                          </span>
                        </td>
                      </tr>
            ...
            <div style={{ background: `${C.teal}11`, border: `1px solid ${C.teal}44`, borderRadius: 12, padding: 16, fontSize: 11, color: C.dim, lineHeight: 1.6 }}>
              💡 <strong style={{ color: C.teal }}>Conclusión:</strong> los raros añaden <strong style={{ color: C.gold }}>${rarePremium}</strong> al costo total — un impacto <strong style={{ color: C.text }}>menor y controlable</strong>. Lo que realmente encarece el álbum es el último 5% de cromos comunes, no los raros.
            </div>
          </div>
        )}
        ...
        {tab === "growth" && (
          <div>
            <div style={{ color: C.gold, fontWeight: 700, fontSize: 14, marginBottom: 4 }}>📈 Únicos vs Repetidos — el punto de inflexión</div>
            <div style={{ background: `${C.orange}11`, border: `1px solid ${C.orange}33`, borderRadius: 8, padding: "8px 14px", marginBottom: 14, fontSize: 11, color: C.dim }}>
              📍 Inflexión: <strong style={{ color: C.orange }}>~{INF} sobres</strong> (${(INF * price).toFixed(0)}) — desde aquí los repetidos superan a los nuevos en cada sobre que abres.
            </div>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 18 }}>
              <ResponsiveContainer width="100%" height={290}>
                <AreaChart data={growthData}>
                  ...
                  <XAxis dataKey="packs" stroke={C.muted} tick={{ fill: C.dim, fontSize: 10 }} label={{ value: "Sobres abiertos", fill: C.dim, fontSize: 10, position: "insideBottom", dy: 10 }} />
                  <YAxis stroke={C.muted} tick={{ fill: C.dim, fontSize: 10 }} />
                  <Tooltip
                    content={({ active, payload, label }) => {
                      if (!active || !payload?.length) return null;
                      return (
                        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px", fontSize: 11 }}>
                  ...
                  <ReferenceLine x={INF} stroke={C.orange} strokeWidth={2.5} strokeDasharray="6 3" label={{ value: `⬆ Inflexión: ${INF} sobres`, fill: C.orange, fontSize: 11, position: "insideTopLeft", fontWeight: 700 }} />
        ...
        {tab === "budget" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 14 }}>
              {[
                ["10%", Math.round(EP * 0.72 * price), "Suerte extrema"],
                ["50%", Math.round(EP * 0.97 * price), "Mediana"],
                ["90%", Math.round(EP * 1.21 * price), "Recomendado"],
                ["99%", Math.round(EP * 1.56 * price), "Casi seguro"],
              ].map(([lbl, val, sub]) => (
                <KPI key={lbl} icon="🎯" label={`P(completar)=${lbl}`} val={`$${val}`} sub={sub} accent={C.blue} small />
              ))}
            </div>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 18 }}>
              <ResponsiveContainer width="100%" height={230}>
                <AreaChart data={budgetData}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
                  <XAxis dataKey="budget" stroke={C.muted} tick={{ fill: C.dim, fontSize: 10 }} tickFormatter={v => `$${v}`} label={{ value: "Presupuesto (USD)", fill: C.dim, fontSize: 10, position: "insideBottom", dy: 10 }} />
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {stageData.map((row, i) => {
                    ...
                    const badge = i < 2 ? "EFICIENTE" : i < 4 ? "COSTOSO" : "⚠️ CRÍTICO";
                    return (
                      <tr key={row.label} style={{ borderTop: `1px solid ${C.border}` }}>
                        ...
                        <td style={{ padding: "9px 14px" }}>
                          <span style={{ background: `${col}22`, color: col, padding: "3px 9px", borderRadius: 8, fontSize: 10, fontWeight: 700 }}>{badge}</span>
                        </td>
                      </tr>
                    );
                  })}
        ...
        {tab === "monte" && (
          <div>
            ...
            <div style={{ color: C.dim, fontSize: 11, marginBottom: 16 }}>Distribución real del costo bajo incertidumbre — ajusta los parámetros y ejecuta</div>

            <div style={{ background: C.card, border: `2px solid ${C.gold}33`, borderRadius: 14, padding: 18, marginBottom: 18 }}>
              <div style={{ fontWeight: 700, color: C.gold, fontSize: 12, marginBottom: 14 }}>⚙️ Número de simulaciones</div>
              <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-end" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontSize: 10, color: C.dim }}>🔁 Cantidad de simulaciones</span>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <input
                      type="number" value={rawSimN} min={100} max={5000} step={100}
                      onChange={e => setRawSimN(e.target.value)}
                      style={{ width: 90, background: "#0f1e35", border: `1px solid ${C.gold}`, borderRadius: 7, color: C.gold, padding: "6px 8px", fontSize: 14, fontWeight: 700, fontFamily: "monospace", outline: "none" }}
                    />
                    <div style={{ display: "flex", gap: 5 }}>
                      {[200, 500, 1000, 2000].map(v => (
                        <button key={v} onClick={() => setRawSimN(String(v))} style={{ padding: "5px 10px", borderRadius: 7, border: `1px solid ${simN === v ? C.gold : C.border}`, background: simN === v ? `${C.gold}22` : "transparent", color: simN === v ? C.gold : C.dim, fontSize: 11, cursor: "pointer", fontWeight: simN === v ? 700 : 400 }}>
                          {v}
                        </button>
                      ))}
                    </div>
                ...
                <div style={{ background: `${C.border}60`, borderRadius: 9, padding: "8px 14px", fontSize: 11, color: C.dim }}>
                  <div>Cromos: <strong style={{ color: C.text }}>{n}</strong> · {spp}/sobre · ${price}</div>
                  <div style={{ fontSize: 10, color: C.muted, marginTop: 3 }}>Cambia en los parámetros del header</div>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 16 }}>
              <button
                onClick={runSim}
                disabled={running}
                style={{ padding: "11px 26px", borderRadius: 10, border: "none", cursor: running ? "wait" : "pointer", background: running ? C.muted : C.gold, color: C.bg, fontWeight: 800, fontSize: 14, boxShadow: running ? "none" : `0 0 16px ${C.gold}55` }}
              >
                {running ? `⏳ Simulando ${simN} álbumes...` : `🎲 Simular ${simN} colecciones`}
              </button>
              ...
              <div>
                ...
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
                  <div style={{ fontSize: 11, color: C.dim, marginBottom: 8 }}>
                    Distribución · {sim.n} simulaciones · {n} cromos · {spp}/sobre · ${price}
                  </div>
                  <SimChart hist={sim.hist} mean={sim.mean} price={price} height={190} />
                </div>
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, display: "grid", gridTemplateColumns: "repeat(5,1fr)" }}>
                  {[
                    ["P5",  sim.p5,     C.green,  "Suerte"],
                    ["P25", sim.p25,    C.blue,   "Q1"],
                    ["P50", sim.median, C.gold,   "Mediana"],
                    ["P75", sim.p75,    C.orange, "Q3"],
                    ["P95", sim.p95,    C.red,    "Difícil"],
                  ].map(([lbl, val, col, desc]) => (
        ...
        {/* Footer */}
        <div style={{ marginTop: 24, padding: "10px 0", borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 10, color: C.muted }}>
